...
module Week07.RPS.Utils
    ...
    , unsafeGetCredentials
    , posixTimeToSlotTestnetConverter
    , randomBuiltinByteString
    ) where
...
unsafeGetCredentials :: Plutus.Address -> (Plutus.PaymentPubKeyHash, Maybe Plutus.StakePubKeyHash)
unsafeGetCredentials addr = fromMaybe (error $ "can't get " ++ show addr ++ " as credentials") $ getCredentials addr
...
unsafeCredentials :: Plutus.Address -> (Plutus.PaymentPubKeyHash, Maybe Plutus.StakePubKeyHash)
unsafeCredentials addr = fromMaybe (error $ "the address corresponds to a script address " ++ show addr) $ getCredentials addr
...
randomBuiltinByteString :: IO BuiltinByteString
randomBuiltinByteString = fromString <$> replicateM 10 (randomRIO ('a', 'z'))