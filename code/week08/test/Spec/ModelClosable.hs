...
import           Week08.TokenSaleClosable                       (TokenSale (..), TSStartSchema, TSUseSchema, startEndpoint, useEndpoints')
...
instance ContractModel TSModel where
    ...
    precondition s (SetPrice v _ _)   = isRunning s v
    precondition s (AddTokens v _ _)  = isRunning s v
    precondition s (BuyTokens v _ _)  = isRunning s v
    precondition s (Withdraw v _ _ _) = isRunning s v
    precondition s (Close v _)        = isRunning s v
        ...
        when (v == w) $ do
            ...
            case m of
                Just t -> do
                    let l = t ^. tssLovelace
                        n = t ^. tssToken
                    deposit w $ lovelaceValueOf l <> Ada.toValue minAdaTxOut <> assetClassValue (tokens Map.! v) n
...
isRunning :: ModelState TSModel -> Wallet -> Bool 
isRunning s' w = maybe False (^. tssClose) $ getTSState' s' w
