...
transition ts s r = case (stateValue s, stateData s, r) of
    ...
    (v, d, AddTokens n)  | n > 0                              -> Just ( mempty
                                                                      , State d $
                                                                        v                                       <>
                                                                        assetClassValue (tsToken ts) n
                                                                      )
    (v, d@(Just p), BuyTokens n)  | n > 0                     -> Just ( mempty
                                                                      , State d $
                                                                        v                                       <>
                                                                        assetClassValue (tsToken ts) (negate n) <>
                                                                        lovelaceValueOf (n * p)
                                                                      )
    (v, d, Withdraw n l) | n >= 0 && l >= 0 &&
...
tsStateMachine ts = mkStateMachine (Just $ tsTT ts) (transition ts) isFinal

{-# INLINABLE isFinal #-}
isFinal :: TSDatum -> Bool
isFinal Nothing = True
isFinal _       = False  
